    ...
    <>
      ...
      <style>{`
        @media (min-width: 900px) {
          ...
          .faq-sticky { position: sticky !important; top: 96px !important; }
        }
      `}</style>
      <main style={{ paddingTop: 80, minHeight: '100vh', paddingBottom: 80 }}>
        <div className="container" style={{ maxWidth: 1100, padding: '0 24px' }}>
          ...
          {/* Two-column layout: main content left, FAQ right */}
          <div className="guide-layout" style={{ display: 'block' }}>
            ...
            <div>
              ...
              <section style={{ marginBottom: 56 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  {[
                    { title: '8×8 Grid Board', content: 'The game board is 8 columns × 8 rows = 64 cells. You place pieces on this board.' },
                    { title: '3-Piece Tray', content: 'Three pieces are always available at the bottom. Select one (click or press 1/2/3) then click the board to place it. Pieces cannot be rotated.' },
                    { title: 'Line Clearing', content: "When a full row (8 blocks) or full column (8 blocks) is completed, it gets cleared. Both can happen simultaneously — that's a COMBO and earns multiplier bonuses." },
                    { title: 'No Gravity', content: 'Unlike Tetris, blocks do NOT fall after a clear. Only the cleared row/column disappears. Remaining blocks stay in place.' },
                    { title: 'Game Over', content: 'Game over when none of the 3 available pieces can fit anywhere on the board. No more moves = session ends.' },
                  ].map(item => (
                    <div key={item.title} style={{
                      ...
                      <div>
                        <div style={{ fontFamily: "'Orbitron', monospace", fontSize: 14, color: '#FFFFFF', marginBottom: 6 }}>
                          {item.title}
                        </div>
                        <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 14, color: '#8888BB', lineHeight: 1.6 }}>
                          {item.content}
                        </div>
                      </div>
                    </div>
                ...
                <div style={{
                  ...
                  {[
                    ...
                    <div key={row.clear} style={{
                      ...
                      <span style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 14, color: '#CCCCCC' }}>{row.clear}</span>
                      <span style={{ fontFamily: "'Orbitron', monospace", fontSize: 14, fontWeight: 700, color: row.color, marginRight: 24, textShadow: `0 0 10px ${row.color}60` }}>{row.mult}</span>
                      <span style={{ fontFamily: "'Orbitron', monospace", fontSize: 13, color: '#8888BB', minWidth: 100, textAlign: 'right' }}>{row.pts}</span>
                    </div>
                  ))}
                </div>
                <div style={{ background: 'rgba(0,245,255,0.04)', border: '1px solid rgba(0,245,255,0.1)', borderRadius: 12, padding: '16px 20px' }}>
                  <div style={{ fontFamily: "'Orbitron', monospace", fontSize: 12, color: '#00F5FF', marginBottom: 8 }}>CHAIN BONUS</div>
                  <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 14, color: '#8888BB', lineHeight: 1.6 }}>
              ...
              <section style={{ marginBottom: 0 }}>
                ...
                <div style={{
                  ...
                  <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 14, color: '#8888BB', marginBottom: 16, lineHeight: 1.6 }}>
                    Every USDC spent on tickets is split automatically by the Solana smart contract:
                  </div>
                  {[
                    { pct: 70, label: 'Prize Pool → Distributed to Top 100 players on the 1st of each month', color: '#00FF88' },
                    { pct: 15, label: 'Team Revenue → Development & operations',                              color: '#00F5FF' },
                    { pct: 10, label: 'Dev Fund → Smart contract audits, marketing, infrastructure',          color: '#FF00FF' },
                    { pct: 5,  label: 'Referral Pool → Rewarded to referrers',                              color: '#FFD700' },
                  ].map(item => (
                    <div key={item.pct} style={{ marginBottom: 12 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                        <span style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 13, color: '#CCCCCC' }}>{item.label}</span>
                        <span style={{ fontFamily: "'Orbitron', monospace", fontSize: 13, fontWeight: 700, color: item.color }}>{item.pct}%</span>
                      </div>
            ...
            {/* ── Right column — FAQ (sticky on desktop) ── */}
            <div className="faq-sticky" style={{ marginTop: 0 }}>
                  ...
                  {FAQ.map(faq => (
                    <details key={faq.q} style={{
                      ...
                      <summary style={{
                        ...
                        {faq.q}
                        <span style={{ color: '#00F5FF', fontSize: 18, flexShrink: 0, marginLeft: 12 }}>+</span>
                      </summary>
                      <div style={{
                        padding: '0 18px 14px',
                        paddingTop: 12,
